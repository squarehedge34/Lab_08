...
```{r}
Rain_Guess <- logistic_reg() %>%
  set_engine("glm") %>%
  fit(RainTomorrow ~ RainToday, data = weather_Portland, family = "binomial")
...
```{r}
Weather_guess<- predict(Rain_Guess,weather_Portland, type = 'prob') %>%
  bind_cols(weather_Portland)

ggplot(Weather_guess, aes(x = .pred_Yes))+
  geom_histogram() 
```

> Hint: how many unique values do the predicted probabilities take? What do these value correspond to?

There are only 2 unique values, these values correspond to the chances of rain happening in the following day.
...
There are no missing values in my case because i removed all NA values at the start of my code. This can also be seen by the fact there was no change in number of values throught the code.
...
```{r}
sampleing <- sample(c(TRUE, FALSE), nrow(weather_Portland), replace = TRUE, prob = c(0.7, 0.3))
training_set <- weather_Portland[sampleing,]
testing_set <- weather_Portland[!sampleing,]
